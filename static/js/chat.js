document.addEventListener('DOMContentLoaded', function() {
    ...
    function addBotMessage(message, citations = null, citation_objects = null, graphics = null) {
        ...
        if ((citation_objects && citation_objects.length > 0) || (citations && citations.length > 0)) {
            ...
            // Use citation_objects if available (preferred) as they contain URLs
            if (citation_objects && citation_objects.length > 0) {
                citation_objects.forEach(citation => {
                    const citationItem = document.createElement('div');
                    citationItem.className = 'citation';
                    
                    if (citation.url) {
                        // Create a clickable link if URL is available
                        citationItem.innerHTML = `<a href="${citation.url}" target="_blank">${citation.source || citation.title || 'Source'}</a>`;
                    } else {
                        // Otherwise just show the source name
                        citationItem.innerHTML = citation.source || citation.title || citation.id || 'Source';
                    }
                    
                    citationsList.appendChild(citationItem);
                });
            } else if (citations && citations.length > 0) {
                // Fall back to simple citations array if no citation_objects
