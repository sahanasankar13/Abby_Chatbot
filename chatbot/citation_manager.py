    ...
    def add_citation_to_text(self,
        ...
        if include_citation and source_id in self.sources:
            ...
            if len(text.split()) > 10 and not self._is_conversational(text):
                ...
                # If source has a URL, include it in the citation
                if source.url and source_id == "custom_link":
                    citation_text = f"(Source: {source.source} - {source.url})"
                else:
                    citation_text = f"(Source: {source.source})"
